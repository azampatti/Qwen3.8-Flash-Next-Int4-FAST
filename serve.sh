...
[ -f "$MODEL_DIR/config.json" ] || { echo "Model not found in $MODEL_DIR -- run ./setup.sh first" >&2; exit 1; }
[ -n "$(ls "$TABLE_DIR" 2>/dev/null)" ] || { echo "n-gram table not found in $TABLE_DIR -- run ./setup.sh first" >&2; exit 1; }
...
if [ "$MTP" != 0 ]; then
  ...
  SPEC=(--speculative-config "{\"method\":\"mtp\",\"num_speculative_tokens\":${MTP},\"model\":\"/models/$(basename "$DRAFT")\"}")
fi
...
echo "Serving $(basename "$MODEL_DIR") -- top-k $TOPK, ${CTX} ctx, MTP ${MTP}${DRAFT_NOTE}, KV $KV_BYTES $KV_DTYPE$([ "$DET_TOPK" = 1 ] && echo ', deterministic top-k') on port $PORT"
...
exec docker run "${RUN[@]}" --name qwen38-flash \
  ...
  -v "$MODELS_DIR":/models:ro -v "$TABLE_DIR":/ple-table:ro "${TPL_MOUNT[@]}" \
  -e VLLM_PLE_MMAP=1 -e VLLM_PLE_MMAP_WORKERS=32 -e VLLM_PLE_MMAP_PREWARM=1 -e VLLM_PLE_MMAP_DIR=/ple-table \
  -e VLLM_MARLIN_USE_ATOMIC_ADD=1 -e VLLM_FP8_HYBRID=1 -e VLLM_USE_DEEP_GEMM=0 -e VLLM_USE_FLASHINFER_SAMPLER=1 "${DET[@]}" \
  "$IMAGE" "/models/$(basename "$MODEL_DIR")" --served-model-name "$SERVED_NAME" \
